...
function parseParams(params, opts) {
  ...
  if (params.fields) result.options.fields = fields.parse(params.fields);
  if (params.offset) result.options.skip = offset(params.offset);
  if (params.limit) result.options.limit = limit(params.limit);
  if (params.sort) result.options.sort = sort.parse(params.sort);
